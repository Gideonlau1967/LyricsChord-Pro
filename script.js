document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- STATE ---
    let setlist = [{ title: "Song Title", lyrics: "[Verse 1]\nG           C\nLyrics here...", copy: "©", shift: 0, align: 'center' }];
    let activeIndex = 0;
    ...
    async function handleImport(event) {
        ...
        for (let file of files) {
            try {
                ...
                setlist.push({ title: extractedTitle || file.name.replace('.pptx',''), lyrics: fullLyrics.trim(), copy: extractedCopy || "©", shift: 0, align: 'center' });
    ...
    // --- BG GALLERY ---
    const bgOptions = [ { name: 'Default', path: 'assets/bg-default.png' }, { name: 'Holy', path: 'assets/bg-HolySpirit.png' }, { name: 'Linen', path: 'assets/bg-linen.png' }, { name: 'Dark', path: 'assets/bg-dark.png' } ];
    const bgSelector = document.getElementById('bgSelector');
    bgOptions.forEach(opt => {
        const thumb = document.createElement('div');
        thumb.className = `bg-thumb ${opt.path === selectedBgPath ? 'active' : ''}`;
        thumb.style.backgroundImage = `url(${opt.path})`;
        thumb.onclick = () => { document.querySelectorAll('.bg-thumb').forEach(t => t.classList.remove('active')); thumb.classList.add('active'); selectedBgPath = opt.path; updatePreview(); };
        bgSelector.appendChild(thumb);
    });

    // --- STATE MANAGEMENT ---
    function renderSetlist() {
        const container = document.getElementById('setlistItems'); container.innerHTML = '';
        ...
        document.querySelectorAll('.del-btn').forEach(btn => {
            btn.onclick = (e) => { e.stopPropagation(); if (setlist.length > 1) { setlist.splice(parseInt(e.target.dataset.idx), 1); activeIndex = 0; loadActiveSong(); } };
        });
    }
    ...
    function loadActiveSong() { const s = setlist[activeIndex]; titleInput.value = s.title; lyricInput.value = s.lyrics; copyInput.value = s.copy; alignInput.value = s.align; document.getElementById('keyShift').innerText = `Shift: ${s.shift}`; renderSetlist(); updatePreview(); }
    ...
    function transposeChord(chord, steps) {
        return chord.replace(/[A-G][b#]?/g, m => {
            ...
            if (i === -1) return m;
            let newI = (i + steps) % 12; while (newI < 0) newI += 12; return SCALE[newI];
        });
    }
    ...
    function updatePreview() {
        ...
        [pt, pc, pl].forEach(el => { el.style.textAlign = align; el.style.fontFamily = MAIN_FONT; el.style.color = document.getElementById('colLyrics').value; });
        pt.innerText = titleInput.value; pt.style.top = document.getElementById('yTitle').value + "%"; pt.style.fontSize = (SIZE_TITLE * scale) + "px"; pt.style.color = document.getElementById('colTitle').value;
        pc.innerText = copyInput.value; pc.style.top = document.getElementById('yCopy').value + "%"; pc.style.fontSize = (SIZE_COPY * scale) + "px"; pc.style.color = document.getElementById('colCopy').value;
        ...
        activeText.split('\n').forEach((line, i, arr) => {
            ...
            if (line.trim().startsWith('[')) { div.style.fontSize = (SIZE_SECTION * scale) + "px"; div.style.fontWeight = "bold"; div.innerText = line; }
                ...
                div.innerHTML = createHtmlLine(line, arr[i+1] || "", scale, align, document.getElementById('colChords').value);
            }
            else { div.style.fontSize = (SIZE_LYRIC * scale) + "px"; div.innerText = line || " "; }
            pl.appendChild(div);
        });
    }
    ...
    async function downloadPptx(all = false) {
        ...
        songs.forEach(song => {
            song.lyrics.split(/(?=\[)/).filter(s => s.trim()).forEach(section => {
                ...
                slide.addText(song.title, { x:"5%", y:document.getElementById('yTitle').value+"%", w:"90%", fontSize:SIZE_TITLE, fontFace:MAIN_FONT, bold:true, align:song.align, color:document.getElementById('colTitle').value.replace('#','') });
                let textObjs = [];
                section.trim().split('\n').forEach((line, i, arr) => {
                    if (line.trim().startsWith('[')) textObjs.push({ text: line+"\n", options: { fontSize:SIZE_SECTION, bold:true } });
                    else if (isChordLine(line)) {
                        const len = song.align === 'center' ? Math.max(line.length, (arr[i+1]||"").length) : line.length;
                        for (let k = 0; k < len; k++) {
                            const c = line[k] || " ", l = (arr[i+1]||"")[k] || " ";
                            if (c !== " ") textObjs.push({ text: c, options: { color: document.getElementById('colChords').value.replace('#',''), fontSize: SIZE_CHORD } });
                            else textObjs.push({ text: l===""?" ":l, options: { transparency: 100, fontSize: SIZE_LYRIC } });
                        }
                        textObjs.push({ text: "\n" });
                    }
                    else textObjs.push({ text: (line||" ")+"\n", options: { fontSize:SIZE_LYRIC, color:document.getElementById('colLyrics').value.replace('#','') } });
                });
                ...
                slide.addText(song.copy, { x:"5%", y:document.getElementById('yCopy').value+"%", w:"90%", fontSize:SIZE_COPY, fontFace:MAIN_FONT, italic:true, align:song.align, color:document.getElementById('colCopy').value.replace('#','') });
            });
        });
    ...
    document.getElementById('btnAddSong').onclick = () => { saveActiveSong(); setlist.push({ title: "New Song", lyrics: "", copy: "©", shift: 0, align: 'center' }); activeIndex = setlist.length - 1; loadActiveSong(); };
    document.querySelectorAll('input, textarea, select').forEach(el => el.addEventListener('input', () => { saveActiveSong(); updatePreview(); if(el.id === 'valTitle') renderSetlist(); }));
    document.getElementById('btnUp').onclick = () => applyTranspose(1);
    document.getElementById('btnDown').onclick = () => applyTranspose(-1);
    document.getElementById('btnGlobalTranspose').onclick = () => { const v = prompt("Steps to shift entire setlist:", "1"); if (v) applyTranspose(parseInt(v), true); };
    ...
    document.getElementById('downloadCurrent').onclick = () => downloadPptx(false);
    document.getElementById('downloadSetlist').onclick = () => downloadPptx(true);
    document.getElementById('exportSession').onclick = () => { saveActiveSong(); const blob = new Blob([JSON.stringify(setlist)], {type: "application/json"}); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = "setlist.json"; a.click(); };
    document.getElementById('importSession').onchange = (e) => { const reader = new FileReader(); reader.onload = (ev) => { setlist = JSON.parse(ev.target.result); activeIndex = 0; loadActiveSong(); }; reader.readAsText(e.target.files[0]); };
    document.getElementById('btnFullScreen').onclick = () => { const m = document.getElementById('slideMock'); if (!document.fullscreenElement) m.requestFullscreen(); else document.exitFullscreen(); };
    loadActiveSong();
});
